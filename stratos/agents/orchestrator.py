...
def scout_node(state: GraphState):
    print("\n=== SCOUT AGENT ===")
    try:
        results = run_scout(state["run_id"], state["db_url"])
...
def analyst_node(state: GraphState):
    ...
    try:
        results = run_analyst(snapshots, state["run_id"], state["db_url"])
...
def strategist_node(state: GraphState):
    ...
    try:
        results = run_strategist(signals, state["run_id"], state["db_url"])
...
def writer_node(state: GraphState):
    ...
    try:
        success = run_writer(results, state["run_id"], state["db_url"])
